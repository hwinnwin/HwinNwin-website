import type { DamageItem, QuoteCalculation } from "@shared/schema";
...
export function validatePhotos(photoCount: number): { isValid: boolean; missing: string[] } {
  const minRequired = 3; // At least 3 photos for basic assessment
  const isValid = photoCount >= minRequired;
  ...
  return {
    isValid: false,
    missing: requiredPhotoAngles.slice(photoCount)
  };
}
